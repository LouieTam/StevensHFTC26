...
def submit_market_buy(trader, symbol, lots):
    if lots <= 0:
        return
...
def submit_market_sell(trader, symbol, lots):
    if lots <= 0:
        return
...
def run_mlofi_pressure_signal_with_execution(trader, symbol=SYMBOL, levels=LEVELS,
    ...
    while datetime.now() < end_time:
        ...
        long_lots_before, short_lots_before, net_pos_before = get_position_lots(trader, symbol)
        ...
        if signal == "BUY_PRESSURE":
            ...
            if net_pos_before < 0:
                cover_lots = abs(net_pos_before)
                submit_market_buy(trader, symbol, cover_lots)
                action_taken = f"EXIT_SHORT MARKET_BUY {cover_lots}"
                time.sleep(0.5)

            else:
                if buy_streak >= 3 and net_pos_before < MAX_NET_POSITION:
                    buy_lots = min(TRADE_LOTS, MAX_NET_POSITION - net_pos_before)
                    if buy_lots > 0:
                        submit_market_buy(trader, symbol, buy_lots)
                        action_taken = f"MARKET_BUY {buy_lots}"
                        time.sleep(0.5)
            ...
            if net_pos_before > 0:
                exit_lots = net_pos_before
                submit_market_sell(trader, symbol, exit_lots)
                action_taken = f"EXIT_LONG MARKET_SELL {exit_lots}"
                time.sleep(0.5)

            else:
                if sell_streak >= 3 and net_pos_before > -MAX_NET_POSITION:
                    sell_lots = min(TRADE_LOTS, MAX_NET_POSITION + net_pos_before)
                    if sell_lots > 0:
                        submit_market_sell(trader, symbol, sell_lots)
                        action_taken = f"MARKET_SELL {sell_lots}"
                        time.sleep(0.5)
        ...
        long_lots_after, short_lots_after, net_pos_after = get_position_lots(trader, symbol)
        ...
        print(f"Long lots before:  {long_lots_before}")
        print(f"Short lots before: {short_lots_before}")
        print(f"Net pos before:    {net_pos_before}")
        print(f"Long lots after:   {long_lots_after}")
        print(f"Short lots after:  {short_lots_after}")
        print(f"Net pos after:     {net_pos_after}")
...
if __name__ == "__main__":
    with shift.Trader("columbia-traders") as trader:
        ...
        try:
            run_mlofi_pressure_signal_with_execution(
                trader,
                symbol=SYMBOL,
                levels=LEVELS,
