...
def close_positions(trader, ticker):
    ...
    if long_shares > 0:
        print(
            f"market selling because {ticker} long shares = {long_shares}", flush=True
        )
        order = shift.Order(
            shift.Order.Type.MARKET_SELL, ticker, int(long_shares / 100)
        )  # we divide by 100 because orders are placed for lots of 100 shares
    ...
    if short_shares > 0:
        print(
            f"market buying because {ticker} short shares = {short_shares}", flush=True
        )
        order = shift.Order(
            shift.Order.Type.MARKET_BUY, ticker, int(short_shares / 100)
        )
        trader.submit_order(order)
        sleep(1)
...
def strategy(trader: shift.Trader, ticker: str, endtime):
    ...
    while trader.get_last_trade_time() < endtime:
        ...
        if midprice > previous_price:  # price has increased since last timestep
            ...
            trader.submit_order(order)
            print(
                f"[ORDER_SUBMITTED] | TYPE: MARKET_BUY | TKR: {ticker} | SIZE: {order_size}",
                flush=True,
            )
            ...
            trader.submit_order(order)
            print(
                f"[ORDER_SUBMITTED] | TYPE: MARKET_SELL | TKR: {ticker} | SIZE: {order_size}",
                flush=True,
            )
...
def main(trader):
    ...
    print(f"final bp: {trader.get_portfolio_summary().get_total_bp()}", flush=True)
    print(
