...
QUOTE_LOTS         = 3
MIN_SPREAD         = 0.04
MAX_SPREAD         = 20.0        # don't sample mid price above this
CYCLE_SECONDS      = 2
POLL_INTERVAL      = 0.2
PNL_INTERVAL       = 1
...
EMPTY_BID_PRICE    = 10.0
EMPTY_ASK_PRICE    = 400.0
BOTH_EMPTY_BID     = 1.0
BOTH_EMPTY_ASK     = 1000.0
MAX_BUYING_POWER   = 1_000_000

ZSCORE_WINDOW      = 40          # valid samples required
ZSCORE_THRESHOLD   = 5.0         # SD threshold for anomaly
MM_PAUSE_SECONDS   = 3.0         # pause MM on anomaly detection

LOG_PATH     = "zi_mm_log_r4_run7.csv"
PNL_LOG_PATH = "zi_mm_pnl_r4_run7.csv"
...
class PriceFilter:
    ...
    A mid price is valid only when:
      - both sides of the book are non-empty
      - spread <= MAX_SPREAD
      - the price is within ZSCORE_THRESHOLD SDs of the current mean
        (requires window already full; anomalous prices are discarded)

    Exposes:
      update(bid, ask)  -> call every tick to try to add a sample
      mm_allowed()      -> True if normal MM quoting is permitted
    """
    def __init__(self, symbol):
        self.symbol         = symbol
        self.window         = deque(maxlen=ZSCORE_WINDOW)
        self.mm_pause_until = 0.0   # wall-clock time until MM is paused

    def update(self, sim_time, bid, ask):
        """
        Try to add (bid+ask)/2 to the window.
        Returns 'ok', 'anomaly', or 'skipped' for logging.
        """
        # Only sample when both sides valid and spread not too wide
        ...
        # If window not full yet, add unconditionally
        if len(self.window) < ZSCORE_WINDOW:
            self.window.append(mid)
            return 'ok'

        # Window full — compute z-score
        mean = statistics.mean(self.window)
        std  = statistics.stdev(self.window)

        if std < 1e-9:
            # All prices identical — accept and move on
            self.window.append(mid)
            return 'ok'

        z = abs(mid - mean) / std
        if z > ZSCORE_THRESHOLD:
            # Anomalous price — discard, reset MM pause timer
    ...
    def mm_allowed(self):
        """
        MM quoting is allowed only when:
          1. Window is full (40 valid samples)
          2. Not currently in a pause window
        """
        if len(self.window) < ZSCORE_WINDOW:
            return False
...
class TickerMM:
    ...
    def _cancel_all_mm_orders(self, trader, sim_time):
        """Cancel both resting MM legs — called when MM is suspended."""
        waiting = self._get_my_waiting(trader)
        for oid, price, side_label in [
            (self.bid_oid, self.bid_price, "BID"),
            (self.ask_oid, self.ask_price, "ASK"),
