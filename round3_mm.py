...
# CONFIG
# ---------------------------------------------------------------------------
SYMBOLS        = ["SHOO", "CROX", "PZZA"]
TICK           = 0.01
LOT_SIZE       = 100
CYCLE_SECONDS  = 5       # check / reprice every N seconds
MAX_CYCLES_LIQ = 60      # after this many cycles on one leg, submit at mid
...
class TickerMM:
    ...
    def _handle_quoting(self, trader, sim_time, pos, bid, ask, mid):
        ...
        # ── One leg filled: move to liquidation ───────────────────────────
        if bid_filled and not ask_filled:
            log(sim_time, self.symbol, "BID_FILLED",
                pos_lots=pos, detail="long 1L — cancelling ask, liquidating")
        ...
        if ask_filled and not bid_filled:
            log(sim_time, self.symbol, "ASK_FILLED",
                pos_lots=pos, detail="short 1L — cancelling bid, liquidating")
            cancel_symbol(trader, self.symbol)
            self.state      = self.LIQUIDATING
            self.liq_cycles = 0
            self.liq_oid    = None
    ...
    # ── LIQUIDATING: one position open, work to close it ─────────────────
    def _handle_liquidating(self, trader, sim_time, pos, bid, ask, mid):
        # If already flat, we're done
        if pos == 0:
        ...
        # Check if current liquidation order is still resting
        if self.liq_oid is not None and order_filled(trader, self.liq_oid):
            # It filled — check position
            if pos == 0:
                log(sim_time, self.symbol, "LIQ_COMPLETE",
                    pos_lots=0, detail="liq order filled → IDLE")
                self.reset()
                return
            else:
                # Partial fill or position changed unexpectedly
                self.liq_oid = None

        # Decide price: 2-tick aggressive or mid after MAX_CYCLES_LIQ
        if self.liq_cycles <= MAX_CYCLES_LIQ:
            if close_side == "SELL":
                price = round_tick(ask - 2 * TICK)
            else:
                price = round_tick(bid + 2 * TICK)
            detail = f"LIQ cycle {self.liq_cycles}/{MAX_CYCLES_LIQ} @ 2tick"
        else:
            price  = mid
            detail = f"LIQ cycle {self.liq_cycles} — AT MID {mid}"

        # Cancel old liq order and reprice
        cancel_symbol(trader, self.symbol)
        self.liq_oid = submit_limit(
            trader, self.symbol, close_side, abs(pos), price,
            sim_time, detail
        )
        log(sim_time, self.symbol, "LIQ_REPRICE",
            side=close_side, price=price, pos_lots=pos,
            detail=detail)
